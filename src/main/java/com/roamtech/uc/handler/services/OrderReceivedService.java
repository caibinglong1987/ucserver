...
public class OrderReceivedService extends AbstractService {
	private class OrderReceivedRequest extends UCRequest {
		private Long orderId;
		private Boolean delete;

		public OrderReceivedRequest(HttpServletRequest request) {
			super(request);
	...
	public void doHandle(String target, Request baseRequest,
		...
		OrderReceivedRequest odSetReq = new OrderReceivedRequest(request);
		...
		if(odSetReq.validate()) {
			if(ucService.isSessionValid(odSetReq.getSessionId())) {
				try {
					order = ucService.receivedOrder(odSetReq.getUserid(),odSetReq.orderId);
