 ...
 */
public class BlackListAddService extends AbstractService{
    @Autowired
    ...
    public void doHandle(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (blackListGetRequest.validate()) {
            if (ucService.isSessionValid(blackListGetRequest.getSessionId())) {
                ...
                result.setPhone(blackListGetRequest.phone);
                result.setUserId(blackListGetRequest.getUserid());
                blackListRepo.save(result);
            } else {
